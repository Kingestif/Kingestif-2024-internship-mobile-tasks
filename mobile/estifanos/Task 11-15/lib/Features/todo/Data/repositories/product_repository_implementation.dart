  ...
  class ProductRepositoryImpl implements ProductRepository {
    ...
    Future<Either<Failure, ProductEntity>> getProductID(String id) async {
      if(await networkInfo.isConnected){
        try{
          final product = await remoteDataSource.getProductById(id);   //get the Product from Remote
        ...
        try{
          final product = await localDataSource.getProduct(id);   //incase no internet fetches product form local
          return Right(product);
        }on CacheFailure {
    ...
    @override
    Future<Either<Failure, ProductEntity>> insertProduct(ProductEntity product) async {
      try {
    ...
    Future<Either<Failure, void>> updateProduct(ProductEntity Product) async {
      try {
        ...
        await remoteDataSource.updateProduct(productId);
